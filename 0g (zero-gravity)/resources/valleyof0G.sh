...
validator node current binaries version: ${CYAN}v2.0.2${RESET}
...
function install_0gchain_app() {
    cd $HOME || return
    echo "Downloading and installing 0gchaind v2.0.2..."
    
    # Download and extract package
    wget -q https://github.com/0glabs/0gchain-ng/releases/download/v2.0.2/galileo-v2.0.2.tar.gz -O galileo-v2.0.2.tar.gz
    tar -xzf galileo-v2.0.2.tar.gz -C $HOME
    ...
    if [ -f "$HOME/galileo/bin/0gchaind" ]; then
        ...
        echo "0gchaind v2.0.2 installed successfully to:"
...
function show_guidelines() {
    ...
    echo "   - Regularly update your nodes to the latest version (currently v2.0.2) to ensure compatibility and security."

    echo -e "${GREEN}6. Option Descriptions and Guides${RESET}"
    echo -e "${GREEN}Validator Node Options:${RESET}"
    echo "   a. Deploy/re-Deploy Validator Node: Sets up a new validator node or redeploys an existing one (v2.0.2)."
    echo "      - Guide: This will install all necessary components. Ensure your system meets requirements."
    echo "   b. Manage Validator Node: Update validator node version (v2.0.2) or return to menu."
    ...
    echo "   5. Install 0gchain App: Installs CLI (v2.0.2) for transactions without running a node."
...
function menu() {
    ...
    echo -e "${GREEN}5. Install the 0gchain App (v2.0.2) only to execute transactions without running a node${RESET}"
